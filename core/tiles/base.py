...
class TileDownloader(object):
    ...
    def __init__(self, top_left, right_bottom, zoom: int):
        ...
        self.start_x, self.start_y, self.end_x, self.end_y = self.tile_xy.get_xy_range(top_left[0], top_left[1], right_bottom[0], right_bottom[1])
    ...
    def download(self, folder, **kwargs):
        ...
        for t in self.tiles:
            t.file = Path(folder) / self.output_format.format(x=t.tile.x, y=t.tile.y, format=self.format)
    ...
    def get_urls(self, top_left: tuple[float, float], right_bottom: tuple[float, float]):
        for tile in self.tile_xy.iter_tile_xy(top_left[0], top_left[1], right_bottom[0], right_bottom[1]):
            yield TileFile(url=self._get_url(tile.x, tile.y), tile=tile)

        
    async def _request_httpx(self, limiter: trio.CapacityLimiter, client: httpx.AsyncClient, t: TileFile) -> TileFile:
        async with limiter:
            try:
...
if __name__ == "__main__":
    ...
    tile = TileDownloader((41.87501349541372, 113.78232363984644), (37.74276146796519, 119.16156979277075), zoom=7)
